...
public interface User extends DeliveryObject {
  ...
  /**
   * Represents whether an account is still valid or if it has been deleted.
   *
   * @author jacksonchaiken
   *
   */
  ...
  /**
   * UserBuilder provides a way to build a User if it must be created for the
   * first time and it should be added to the database.
   *
   * @author jacksonchaiken
   *
   */
    ...
    public User build() {
      if (id == null || name == null || paymentId == null || cell == null
        ...
        throw new IllegalArgumentException(
            "There are parameters that have" + " not been set");
      }
      final UserBean user = new UserBean(id, name, paymentId, cell, pass,
          status);
   ...
   */
  void charge(double amount, List<String> items, String pickup,
      String destination);
   ...
   *
   * @param distance
   *          the maximum distance they are willing to travel.
   * @param minimumFee
   *          the minimum fee they wish to receive from the delivery
   * @param maximumTime
   *          the most time they want to spend on the delivery
   */
  void addDeliveryPreferences(double distance, double minimumFee,
      double maximumTime);
  ...
  /**
   * Returns the maximum amount of time the user wants to spend, or -1 if it has
   * not been set.
   *
   * @return the preference.
   */
   ...
   * @param newPassword
   *          the new password.
   * @return true, if the old passwords matched and reset occurred. False
   *         otherwise.
   */
  ...
  /**
   * Returns true or false if the email/password combination matches an existing
   * user's information.
   *
   * @param email
   *          the user's email
  ...
  /**
   * Updates a users password. Should only be used when resetting a password.
   * @param id the user's id.
   * @param newPassword the new password.
   * @return true, if the password could be updated. False otherwise.
   */
  ...
  /**
   * Returns true if an email cell combination is attached to a user.
   * @param email the user's email.
   * @param cell the user's phone number.
   * @return true, if the user exists. False otherwise.
   */
  static boolean resetCombination(String email, String cell) {
    return UserProxy.resetCombination(email, cell);
  }
}
