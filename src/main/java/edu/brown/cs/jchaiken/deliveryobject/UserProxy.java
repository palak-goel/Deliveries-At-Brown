...
class UserProxy extends DeliveryObjectProxy<User> implements User {
    ...
    try (PreparedStatement cachePrep = Database.getConnection()
      ...
      try (ResultSet rs = cachePrep.executeQuery()) {
        if (rs.next()) {
          ...
          final String cell = rs.getString(3);
          final int pass = rs.getInt(4);
          final String stripe = rs.getString(5);
          final String url = rs.getString(6);
          final UserBuilder bean = new UserBuilder();
          ...
          final List<Double> oRatings = new ArrayList<>();
          final List<Double> dRatings = new ArrayList<>();
          ...
          final User newUser = bean.setCell(cell).setPayment(stripe)
              .setId(super.getId()).setName(name).setPersonalUrl(url)
              .setPassword(pass).setDelivererRatings(dRatings)
          ...
          try (PreparedStatement orderPrep = Database.getConnection()
            ...
            try (ResultSet orders = orderPrep.executeQuery()) {
              while (orders.next()) {
                final Order order = Order.byId(orders.getString(1));
  ...
  @Override
  public void charge(double amount, List<String> items, String pickup,
      String destination) {
    check();
    if (super.getData() == null) {
      return;
  ...
  protected static boolean resetPassword(String id, String oldPassword,
      String newPassword) {
    final int pass = newPassword.hashCode();
    if (!User.byId(id).validatePassword(oldPassword)) {
      return false;
    }
    try (PreparedStatement prep = Database.getConnection()
        .prepareStatement(PASS_RESET)) {
      prep.setInt(1, pass);
      prep.setString(2, id);
      prep.executeUpdate();
  ...
  private static final String COMBO_Q
      = "SELECT * FROM users WHERE id = ? AND cell = ?";

  protected static boolean resetCombination(String email, String cell) {
    try (PreparedStatement prep = Database.getConnection()
        .prepareStatement(COMBO_Q)) {
      prep.setString(1, email);
      prep.setString(2, cell);
      try (ResultSet rs = prep.executeQuery()) {
        if (rs.next()) {
          return true;
        }
